...
del.ids <- read.table("../data/input/omni_qc_sample_check_delete.txt", stringsAsFactors = F)
...
out.map <- out.map[,1:3]
write.csv(out.map, "../data/output/qc_identity_check_sample_map.txt", quote=F, row.names=F)
